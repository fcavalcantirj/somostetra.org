-- Fix uuid_generate_v4() calls in trigger by using gen_random_uuid() instead
-- gen_random_uuid() is built-in to PostgreSQL and doesn't require extensions
  ...
  IF user_type_val = 'supporter' THEN
    BEGIN
      INSERT INTO public.supporters (
        ...
        gen_random_uuid(),  -- FIXED: Use gen_random_uuid() instead of uuid_generate_v4()
        NEW.id,
        NEW.email,
        COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email),
        referrer_id,
...
SELECT '✓ Trigger updated with gen_random_uuid() fix' as status;
