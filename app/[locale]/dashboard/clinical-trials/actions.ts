...
export async function searchClinicalTrials(
  ...
  const t = await getTranslations("clinicalTrialsValidation")
  try {
    ...
    return { success: false, error: t("apiError") }
  }
}
...
export async function getTrialById(
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, error: t("notFound", { nctId }) }
    }
    ...
    return { success: false, error: t("detailsError") }
  }
}
...
export async function saveSearchQuery(
  ...
  try {
    ...
    if (error) {
      ...
      return { success: false, error: t("saveSearchError") }
    }
