...
export default async function VoteDetailPage({ params }: { params: Promise<{ id: string; locale: string }> }) {
  const { id } = await params
  const t = await getTranslations("voteDetail")
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-border/50">
                <p className="text-sm text-muted-foreground font-bold">
                  ...
                  {t("votes", { count: vote.vote_count })}
                </p>
                <p className="text-sm text-muted-foreground font-bold">
                  {t("status")}: <span className="text-foreground">{vote.status === "active" ? t("statusActive") : t("statusClosed")}</span>
                </p>
              </div>
            </div>
            ...
            {hasVoted ? (
              <div className="glass p-8 rounded-2xl space-y-4 border-2 border-primary/20">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-2xl font-black">{t("thankYou")}</h3>
                </div>
                <p className="text-lg text-muted-foreground">
                  {t("thankYouDescription", { points: 5 })}
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                <div className="glass p-8 rounded-2xl space-y-4">
                  <h3 className="text-2xl font-black">{t("participate")}</h3>
                  <p className="text-lg text-muted-foreground">
                    {t("participateDescription", { points: 5 })}
                  </p>
                </div>
