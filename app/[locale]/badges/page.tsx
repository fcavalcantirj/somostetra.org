        ...
        <div className="container mx-auto max-w-5xl">
          <div className="mb-16 space-y-6">
            ...
            <p className="text-xl text-muted-foreground">
              {t("badgeCount", { earned: earnedBadgeIds.size, total: allBadges?.length || 0 })}
            </p>
          </div>
          ...
          <div className="mb-12 glass-strong p-8 rounded-3xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-2">{t("yourPoints")}</p>
                <p className="text-5xl font-black text-gradient">{profile?.points || 0}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-2">{t("nextBadge")}</p>
                <p className="text-2xl font-black">
                  {allBadges?.find((b) => b.points_required > (profile?.points || 0))?.points_required || t("maxPoints")} pts
                </p>
              </div>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    {isEarned && <Badge className="gradient-primary font-bold">{t("earned")}</Badge>}
                    {!isEarned && canEarn && <Badge className="gradient-accent font-bold">{t("available")}</Badge>}
                  </div>
                  ...
                  <div className="pt-4 border-t border-border/50">
                    <p className="text-sm font-bold text-muted-foreground">
                      {t("requires", { points: badge.points_required })}
                    </p>
                  </div>
                </div>
              )
