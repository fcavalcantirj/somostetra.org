...
export async function checkUsernameAvailability(username: string, currentUserId: string): Promise<{
  ...
  if (!username) {
    return { available: false, message: "Nome de usuário é obrigatório" }
  }

  // Validate format: lowercase alphanumeric and underscore only
  const usernameRegex = /^[a-z0-9_]+$/
  if (!usernameRegex.test(username)) {
    return { available: false, message: "Use apenas letras minúsculas, números e underscore" }
  }

  if (username.length > 100) {
    return { available: false, message: "Máximo 100 caracteres" }
  }
  ...
  if (reserved.includes(username)) {
    return { available: false, message: "Este nome está reservado" }
  }
...
export async function saveProfile(formData: ProfileFormData): Promise<{
  ...
  if (formData.cep && !/^\d{5}-?\d{3}$/.test(formData.cep)) {
    return { success: false, error: "CEP inválido. Use o formato 00000-000" }
  }
  ...
  if (isFirstCompletion && isComplete) {
    ...
    await serviceClient
      ...
      .insert({
        ...
        points: 50,
        description: "Completou o perfil"
      })
